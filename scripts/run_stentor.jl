...
fold_name=datasets.dataset[133]
...
tiled_save=h5open("$MASTER_FOLDER/data/$fold_name/$(fold_name)_tiled.h5")
tiled_save["cell_locs"] = [(floor(Int,x),floor(Int,y)) for (x,y) in mouse_pos]
tiled_save["tiled_frames"] = tiled
write2video(tiled[:,:,:],"/home/para/data/$fold_name/$fold_name";fps=10)
close(tiled_save)
